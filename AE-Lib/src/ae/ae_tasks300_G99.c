...
void test1_start(int test_id, int test_id_data)
{
    ...
    (*p_index)++;
    strcpy(g_ae_xtest.msg, "creating a giant mailbox of size 6969420 Bytes");
    mbx_t mbx_id1 = mbx_create(6969420);
    sub_result = (mbx_id1 < 0) ? 1 : 0;
    process_sub_result(test_id, *p_index, sub_result);

    (*p_index)++;
    sprintf(g_ae_xtest.msg, "creating a small mailbox of size %u Bytes", 1);
    mbx_t mbx_id2 = mbx_create(1);
    sub_result = (mbx_id2 < 0) ? 1 : 0;
    process_sub_result(test_id, *p_index, sub_result);

    (*p_index)++;
    sprintf(g_ae_xtest.msg, "creating repeat mailbox of size %u Bytes", 128);
    mbx_t mbx_ida = mbx_create(128);
    mbx_t mbx_idb = mbx_create(128);
    sub_result = (mbx_idb < 0) ? 1 : 0;
    process_sub_result(test_id, *p_index, sub_result);
